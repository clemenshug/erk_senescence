...
temporal_lfc_ERKi_sum <- temporal_lfc %>%
  ...
  summarize(
    ...
    log2FoldChange_q33 = Hmisc::wtd.quantile(log2FoldChange, probs = 0.33, weights = log2FoldChange_range_erki),
    log2FoldChange_q67 = Hmisc::wtd.quantile(log2FoldChange, probs = 0.67, weights = log2FoldChange_range_erki),
    log2FoldChange_sum_cmap = if_else(
      abs(log2FoldChange_q33) > abs(log2FoldChange_q67),
      log2FoldChange_q33, log2FoldChange_q67
    ),
    ERKi = "all",
    .groups = "drop"
  ) %>%
...
temporal_ordering_syn <- synMkdir("syn21432134", "temporal_ordering_fixed_function")
